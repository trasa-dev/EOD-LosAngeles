...
Public Class ModificarPersona
    ...
    Private primeraCargaTrabajo As Boolean = True
    Private primeraCargaEstudio As Boolean = True

    Public Sub New(ByVal hogar As Integer, ByVal idpersona As Integer)
        InitializeComponent()
        Me.hogar = hogar
        ...
        Me.persona = datasetEOD.Persona.FindByHogarPersona(hogar, idpersona)
        Me.validadorPersona = datasetEOD.ValidaPersona.FindByHogarPersona(hogar, idpersona)
    End Sub
    ...
    Private Sub btnGuardarPersona_Click(sender As Object, e As EventArgs) Handles btnGuardarPersona.Click
        If personaCompleta() Then
            ...
            If lkpRelacion.EditValue = 6 AndAlso Not persona.Actividad.Contains("1") Then
            ...
            If persona.Actividad.Contains("1") Then
                persona.ComunaTrabajo = Me.lkpComunaTrabajo.EditValue
                ...
                ElseIf persona.ComunaTrabajo = 1 Then
                    persona.ComunaTrabajoOtra = Me.txtComunaTrabajoOtra.Text
                End If
                persona.JornadaTrabajo = Me.lkpJornadaTrabajo.EditValue
            Else
            ...
            Try
                ...
                MessageBox.Show("Persona modificada exitosamente", "Datos guardados", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Me.Dispose()
            Catch ex As Exception
                MessageBox.Show("Error al modificar persona. Intente nuevamente" + vbCrLf + vbCrLf + ex.Message + vbCrLf + vbCrLf + ex.StackTrace, "Datos no guardados", MessageBoxButtons.OK, MessageBoxIcon.Error)
            End Try
        Else
            MessageBox.Show("No ha contestado las preguntas mínimas para terminar con la modificación de la persona. Por favor, complete los campos marcados con rojo.", "Datos insuficientes", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
        End If
    ...
    Private Sub cargaPersonaBackground_RunWorkerCompleted(sender As Object, e As System.ComponentModel.RunWorkerCompletedEventArgs) Handles cargaPersonaBackground.RunWorkerCompleted
        ...
        Me.txtNombrePila.Text = persona.NombrePila
        Me.txtAnoNacimiento.Text = persona.AnoNac.ToString
        Me.lkpSexo.EditValue = persona.Sexo
        Me.lkpRelacion.EditValue = persona.Relacion
        If persona.Relacion = 0 Then
            Me.lkpRelacion.Enabled = False
        End If
        Me.lkpEstudios.EditValue = persona.Estudios
        Select Case persona.Estudios
            Case 3, 4, 5
                Me.lkpEstudiosCompletos.EditValue = persona.EstudiosCompletos
        End Select
        If DateTime.Now.Year - persona.AnoNac >= 18 Then
            Me.lkpLicencia.EditValue = persona.LicenciaConducir
        End If
        Me.chkActividad.SetEditValue(persona.Actividad)
        chkActividad_EditValueChanged(Nothing, Nothing)
        If persona.Actividad.Contains("6") AndAlso Not persona.IsActividadOtraNull Then
        ...
        'Datos de trabajo
        If persona.Actividad.Contains("1") Then
            lkpComunaTrabajo.EditValue = persona.ComunaTrabajo
            If persona.ComunaTrabajo = 2 Then
                If Not persona.IsNombreCalle1TrabajoNull Then
                    lkpCalle1Trabajo.EditValue = persona.NombreCalle1Trabajo
                End If
                If Not persona.IsNumeroOCalle2TrabajoNull Then
                    lkpCalle2Trabajo.EditValue = persona.NumeroOCalle2Trabajo
                    Dim args As DevExpress.XtraEditors.Controls.CloseUpEventArgs = New DevExpress.XtraEditors.Controls.CloseUpEventArgs(Nothing)
                    args.AcceptValue = True
                    args.Value = persona.NumeroOCalle2Trabajo
                    lkpCalle2Trabajo_CloseUp(lkpCalle2Trabajo, args)
                End If
                If Not persona.IsHitoTrabajoNull Then
                ...
                If Not persona.IsDirActividadCoordXNull Then
                    x_trabajo.Text = persona.DirActividadCoordX.ToString
                Else
                    x_trabajo.Text = "0"
                End If

                If Not persona.IsDirActividadCoordYNull Then
                    y_trabajo.Text = persona.DirActividadCoordY.ToString
                Else
                    y_trabajo.Text = "0"
                End If

            ElseIf persona.ComunaTrabajo = 1 Then
                txtComunaTrabajoOtra.Text = persona.ComunaTrabajoOtra
            End If
            lkpJornadaTrabajo.EditValue = persona.JornadaTrabajo
        End If

        'Datos de estudio
        If persona.Actividad.Contains("2") Then
            Me.lkpComunaEstudio.EditValue = persona.ComunaEstudio
            If persona.ComunaEstudio = 2 Then
                If Not persona.IsNombreCalle1EstudioNull Then
                    lkpCalle1Estudio.EditValue = persona.NombreCalle1Estudio
                End If
                If Not persona.IsNumeroOCalle2EstudioNull Then
                    lkpCalle2Estudio.EditValue = persona.NumeroOCalle2Estudio
                    Dim args As DevExpress.XtraEditors.Controls.CloseUpEventArgs = New DevExpress.XtraEditors.Controls.CloseUpEventArgs(Nothing)
                    args.AcceptValue = True
                    args.Value = persona.NumeroOCalle2Estudio
                    lkpCalle2Estudio_CloseUp(lkpCalle2Estudio, args)
                End If
                If Not persona.IsHitoEstudioNull Then
                ...
                If Not persona.IsDirEstudiosCoordXNull Then
                    x_estudio.Text = persona.DirEstudiosCoordX.ToString
                Else
                    x_estudio.Text = "0"
                End If

                If Not persona.IsDirEstudiosCoordYNull Then
                    y_estudio.Text = persona.DirEstudiosCoordY.ToString
                Else
                    y_estudio.Text = "0"
                End If
            ElseIf persona.ComunaEstudio = 1 Then
                If Not persona.IsComunaEstudioOtraNull Then
                    Me.txtComunaEstudioOtra.Text = persona.ComunaEstudioOtra
                End If
            End If

            Me.lkpDondeEstudia.EditValue = persona.DondeEstudia
            If Me.lkpDondeEstudia.EditValue = 5 Then
                Me.txtDondeEstudiaOtro.Text = persona.DondeEstudiaOtro
            End If
            Me.lkpTieneTNE.EditValue = persona.PaseEscolar
        End If
    ...
    Private Sub lkpRelacion_EditValueChanged(sender As Object, e As EventArgs) Handles lkpRelacion.EditValueChanged
        If lkpRelacion.EditValue IsNot Nothing AndAlso lkpRelacion.EditValue.ToString <> "" Then
            ...
            If relacion = 0 Then
                ...
                jefehogar = Me.datasetEOD.Persona.Select("hogar = " & Me.hogar & " and relacion = 0 " & " and persona <> " & persona.Persona)
            ...
            If relacion = 1 Then
                ...
                conyuge = Me.datasetEOD.Persona.Select("hogar = " & Me.hogar & " and relacion = 1 " & " and persona <> " & persona.Persona)
    ...
    Private Sub lkpEstudiosCompletos_EditValueChanged(sender As Object, e As EventArgs) Handles lkpEstudiosCompletos.EditValueChanged
        If txtAnoNacimiento.Text <> "" AndAlso lkpEstudios.EditValue IsNot Nothing Then
            ...
            If lkpEstudiosCompletos.EditValue IsNot Nothing AndAlso lkpEstudiosCompletos.EditValue.ToString <> "" Then
                Dim opcion As Integer = Integer.TryParse(lkpEstudiosCompletos.EditValue.ToString, 0)
    ...
    Private Sub btnIngresos_Click(sender As Object, e As EventArgs) Handles btnIngresos.Click
        If persona.EncuestadoLab OrElse persona.EncuestadoSab OrElse persona.EncuestadoDom Then
            Dim ventanaIngresos As New Ingresos(Me.hogar, Me.idpersona)
            ventanaIngresos.Show()
            Me.Dispose()
        Else
            MessageBox.Show("Debe contestar primero la Encuesta de Viajes, antes de reportar sus ingresos", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End If
    End Sub
